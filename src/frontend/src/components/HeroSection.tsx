...
import { BookOpen, Calendar, GraduationCap, Star, Users } from "lucide-react";
...
const CounterCard: React.FC<{
  ...
  return (
    ...
    >
      <div
        className={`${compact ? "" : "mb-2"} p-2 rounded-full flex-shrink-0`}
...
const HeroSection: React.FC = () => {
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(135deg, #3A0A0A 0%, #6B1A1A 40%, #8B2A2A 70%, #4A1010 100%)",
        paddingTop: "80px",
      }}
    >
      ...
      <div
        ...
        style={{ background: "var(--sage)", filter: "blur(30px)" }}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
        ...
        >
          ...
          <h2
            className="text-xl sm:text-2xl lg:text-3xl font-bold text-white/90 mb-4"
            style={{ fontFamily: "Merriweather, serif" }}
          >
            {t("hero.headline2")}
          </h2>
        ...
        {/* Countdown Timer */}
        <div
          className={`mb-10 transition-all duration-1000 delay-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          <p
          ...
          <div className="flex justify-center gap-3 sm:gap-6">
            {countdownBoxes.map((box) => (
              <div
                key={box.label}
                className="countdown-box flex flex-col items-center px-4 py-3 sm:px-6 sm:py-4 min-w-[70px] sm:min-w-[90px]"
              >
                <span
                  className="text-3xl sm:text-4xl lg:text-5xl font-black text-white tabular-nums"
                  style={{ fontFamily: "Merriweather, serif" }}
                >
        ...
        {/* CTA Buttons */}
        <div
          className={`flex flex-col sm:flex-row gap-4 justify-center transition-all duration-1000 delay-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          <a
